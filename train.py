...
def preprocessing(df):
    ...
    def encode(df):
        ...
        df[bin_cols]=oencoder.fit_transform(df[bin_cols])
        return df
    df=encode(df)
...
file=open('da_model.pkl','wb')


pickle.dump(imputer1,file)
pickle.dump(imputer2,file)
pickle.dump(scaler,file)
pickle.dump(oencoder,file)
pickle.dump(sencoder,file)
pickle.dump(knnimputer,file)
pickle.dump(lr,file)
